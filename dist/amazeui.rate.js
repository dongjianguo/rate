(function($) {
  ...
  $.fn.rate = function(config) {
    ...
    $(anchor).attr({class: 'am-rate-anchor', href: 'javascript:;'});
    ...
    //将生成的html结构填充到响应的容器内
    self.find('.am-rate-wrapper').html(htmlList);

    //设置初始化的评分等级
    if(config.init!==0 && config.init <= config.arrayData.length) {
      var index = config.init;
      self.find('.am-rate-anchor').removeClass('active');
      self.find('.am-rate-anchor:lt(' + index + ')').addClass('active');
      var content = config.arrayData[index-1];
      self.find('.am-rate-text').html(content);
    }

    //绑定点击事件
    $('.am-rate-wrapper').on('click', '.am-rate-anchor', function() {
      ...
      //找到最近的父级元素.am-rate-wrapper,并将子元素按钮.am-rate-anchor的并让点击的加上active类名
      var wrapper = $(this).closest('.am-rate-wrapper'); 
      wrapper.find('.am-rate-anchor').removeClass('active');
      wrapper.find('.am-rate-anchor:lt(' + index +')').addClass('active');

      //把点击的相应的文字描述赋值到.am-rate-text文本内
      var content = $(this).data('content');
      $(this).parent('.am-rate-wrapper').siblings('.am-rate-text').html(content);
